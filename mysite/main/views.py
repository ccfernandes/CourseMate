...
def review(request):
	...
	if request.method == "POST":
		...
		if my_form.is_valid():
			...
			my_form.save()
			return render(request=request, template_name="main/course.html", context={'my_form': my_form})
		else:
...
def SearchPage(request):
	...
	# return render(request=request, #so u can reference things inside your template to do with the user
 #                    template_name="main/search.html", context={"form": my_form})
 	queryset=SchoolCourse.objects.order_by('name')
 	return render(request, "main/search.html", context={"object_list": queryset})
